...
function requestRecord(recordId) {
        var formId = 'requestForm_' + recordId;
        document.getElementById(formId).elements['u1'].value = recordId;
        document.getElementById(formId).submit();
    }
</script>
    ...
    <table cellpadding="12" id="customers">
        ...
        <% do { %>
            <tr>
                ...
                <td>
                    <form id="editForm_<%= rs.getString(1) %>" action="updaterequest.jsp" method="post">
                ...
                <td>
                    <form id="deleteForm_<%= rs.getString(1) %>" action="requestForBloodDelete.jsp" method="post">
                        <input type="hidden" name="u1" value="<%= rs.getString(1) %>">
                        <button class="B" style="color:black" type="button" onclick="deleteRecord('<%= rs.getString(1) %>')">Delete</button>
                    </form>
                </td>
                <td>
                    <form id="requestForm_<%= rs.getString(1) %>" action="request.jsp" method="post">
                        <input type="hidden" name="u1" value="<%= rs.getString(1) %>">
                        <button class="B" style="color:black" type="button" onclick="requestRecord('<%= rs.getString(1) %>')">Complete</button>
                    </form>
                </td>
            </tr>
        <% } while(rs.next()); %>
